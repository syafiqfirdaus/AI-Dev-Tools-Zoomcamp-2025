...
class TestCompoundInterest:
    def test_basic_calculation(self):
        ...
        result = calculate_compound_interest(
            principal=10000, rate=0.05, time=10, compounds_per_year=12
        )
    ...
    def test_annual_compounding(self):
        ...
        result = calculate_compound_interest(
            principal=1000, rate=0.10, time=5, compounds_per_year=1
        )
    ...
    def test_zero_rate(self):
        ...
        result = calculate_compound_interest(
            principal=1000, rate=0.0, time=10, compounds_per_year=12
        )
...
class TestLoanAmortization:
    def test_basic_loan(self):
        ...
        result = calculate_loan_amortization(principal=300000, annual_rate=0.045, term_years=30)
    ...
    def test_short_term_loan(self):
        ...
        result = calculate_loan_amortization(principal=10000, annual_rate=0.06, term_years=2)
    ...
    def test_zero_interest_loan(self):
        ...
        result = calculate_loan_amortization(principal=12000, annual_rate=0.0, term_years=1)
...
class TestInvestmentReturn:
    def test_positive_return(self):
        ...
        result = calculate_investment_return(initial_value=10000, final_value=15000, years=5)
    ...
    def test_negative_return(self):
        ...
        result = calculate_investment_return(initial_value=10000, final_value=8000, years=3)
        assert result["roi"] == pytest.approx(-20.0, rel=1e-2)
        assert result["total_return"] == -2000.0

    def test_no_change(self):
        """Test when value doesn't change"""
        result = calculate_investment_return(initial_value=10000, final_value=10000, years=5)
...
class TestRiskMetrics:
    def test_basic_risk_metrics(self):
        ...
        result = calculate_risk_metrics(returns=returns, risk_free_rate=0.02)
    ...
    def test_negative_risk_free_rate(self):
        """Negative policy rates are valid financial inputs."""
        result = calculate_risk_metrics([0.001, 0.002, -0.001], -0.01, periods_per_year=12)
        assert result["sharpe_ratio"] > 0

    def test_frequency_is_explicit(self):
        """The same periodic series annualizes differently at different frequencies."""
        returns = [0.01, 0.02, -0.01, 0.015]
        monthly = calculate_risk_metrics(returns, 0.02, periods_per_year=12)
        daily = calculate_risk_metrics(returns, 0.02, periods_per_year=252)
        assert daily["volatility"] > monthly["volatility"]
        assert monthly["periods_per_year"] == 12
        assert monthly["observations"] == 4
